...
def capture_image(section, part):
    ...
    if not ret:
        print(f"Failed to capture image for {section} - {part}.")
...
def detect_object(frame):
    ...
    print("Detection results:", decoded_predictions)
...
def perform_inspection():
    ...
    for part_name, questions in inspections:
        ...
        while True:
            ...
            # Remove cv2.imshow() and related GUI code
            # Instead of displaying the frame, use debugging prints
            print("Captured Frame for Checking Part")
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):  # Allow user to exit with 'q'
                break
